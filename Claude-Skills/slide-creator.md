...
| セクション見出し（h2）       | `clamp(30px, 3.2vw, 40px)`  | 700         | `var(--text-primary)`   |
| スライドサブタイトル         | `clamp(15px, 1.6vw, 19px)`  | 400         | `var(--text-secondary)` |
| 本文・カード本文・リスト本文 | `clamp(13px, 1.35vw, 17px)` | 400         | `var(--text-primary)`   |
| キャプション・補足           | `clamp(11px, 1.1vw, 14px)`  | 400         | `var(--text-muted)`     |

- **本文色の原則**：カード本文・リストアイテム・説明文は `var(--text-primary)` を基本とする。`--text-secondary` はサブタイトル・補足情報のみに限定する（スライドはモニター距離から見るため、薄いグレーは可読性が低い）
- line-height：本文 1.65〜1.75、リスト・チェックリスト 1.5
...
| 場所                                          | 値                               |
| --------------------------------------------- | -------------------------------- |
| 縦並びスライドのパディング（`.col` クラス等） | `3.5%`                           |
| 分割レイアウト（左右パネル）のパディング      | `4〜4.5%`                        |
| カードパディング                              | `20px 20px 18px`（上/左右 / 下） |
| カードアイコン直径                            | `42px`（`border-radius: 50%`）   |
| 番号バッジ直径                                | `30px`（`border-radius: 50%`）   |
| カード間・リストアイテム間ギャップ            | `9〜12px`                        |
...
- チップ（eyebrow）・バー・ボタンはピル形：`border-radius: 999px`
